...
def build_guideline_jbp(case, records):
    ...
    for c in cp:
        if "▶" in c:
            todos.append(c.split("▶", 1)[1].strip())
...
# 질병별 '사업장에 사실조회할 항목' (담당자가 사업장 조회 시 바로 활용)
SITE_SURVEY = {
    "근골격계질환(척추질환 제외)": [
        "재해자의 담당 공정·작업 내용 및 1일 작업시간",
        "취급 중량물의 무게·빈도, 반복동작 횟수 등 작업 강도",
        "부서 이동 이력 및 각 부서의 신체부담 정도",
        "동료 근로자의 동일 상병 발생 여부",
    ],
    "근골격계질환 (척추질환)": [
        "중량물 취급·허리 굴신 작업의 빈도와 자세",
        "차량 운전 등 전신 진동 노출 여부와 시간",
        "작업 공정별 척추 부담 정도",
        "부서 이동 이력",
    ],
    "호흡기질환(천식 포함)": [
        "작업환경측정 결과(분진·흄·유해가스 농도)",
        "취급 유해인자 종류 및 노출 공정·시간",
        "국소배기장치·환기설비 설치 및 가동 여부",
        "호흡보호구 지급·착용 실태",
        "동일 공정 근로자의 호흡기 질환 발생 여부",
    ],
    "진폐": [
        "분진작업 종사 기간 및 공정",
        "작업환경측정(분진 농도) 결과",
    ...
    "난청": [
        "작업장 소음 수준(작업환경측정 결과, dB)",
        "소음 노출 공정 및 1일 노출시간·총 종사기간",
        "청력보호구 지급·착용 실태",
        "정기 특수건강진단(청력) 결과 추이",
    ],
    "악성신생물(직업성 암 포함)": [
        "발암물질(석면·벤젠·결정형유리규산 등) 취급 공정·기간",
        "작업환경측정 결과 및 노출 농도",
        "보호구 지급 및 환기설비 실태",
    ],
    "뇌혈관질환": [
        "발병 전 12주·4주간 주당 근무시간(초과근무 포함)",
        "야간·교대근무 일정 및 빈도",
        "업무량 변화·돌발상황 유무",
        "근태기록·출퇴근기록 원본",
    ],
    "심장질환": [
        "발병 전 12주·4주간 주당 근무시간",
        "야간·교대근무 및 휴게 실태",
        "업무 스트레스 요인",
        "근태기록 원본",
    ],
    "정신질환": [
        "직장 내 괴롭힘·폭언·폭행 등 사건 경위와 일자",
        "업무량·근무환경 변화",
        "동료·상급자 진술 확보 가능 여부",
        "인사·징계·민원 기록",
    ],
    "간질환": [
        "취급 화학물질·유기용제 종류 및 MSDS(물질안전보건자료)",
        "유해물질 노출 공정·작업시간·환기설비 실태",
        "작업환경측정 결과(유기용제 농도)",
        "보호구 지급·착용 실태 및 동료 근로자 간질환 발생 여부",
    ],
}


def _site_survey_items(disease_group):
    return SITE_SURVEY.get(disease_group, [
        "재해자의 담당 작업 내용 및 작업 강도",
        "유해요인 노출 공정·시간",
        "작업환경측정 결과(해당 시)",
        "동료 근로자의 동일 상병 발생 여부",
    ])
...
ADVISORY_DOCS = {
    ...
    "간질환": ["간기능검사(AST/ALT) 추이", "복부 영상(초음파/CT) 소견", "간염 바이러스 표지자 검사", "취급 유기용제·화학물질 노출력 및 음주력"],
}
